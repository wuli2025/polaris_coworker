...
关键工程点:CloakBrowser 的 launch() 是同步 Playwright,会在调用线程保留运行中的事件循环。
所以本脚本让 main() 同步持有 CloakBrowser/等 CDP/最终关闭，browser-use 的 asyncio.run 则放进
独立工作线程；两条 loop 物理隔离，异步侧只通过 CDP URL 连接，不碰 cloakbrowser 句柄。
...
def ensure_browser_use():
    try:
        ...
        log("未装 browser-use,正在安装固定版本(pip install browser-use==0.13.8)…")
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-q", "browser-use==0.13.8"], check=True
        )
...
def write_result(history, out_dir):
    """保存 browser-use 结果；没有最终结论时保留诊断并让 CLI 失败。"""
    ...
    if final is None:
        raise SystemExit(f"browser-use 未返回最终结果；诊断已保存:{result_path}")
    return final, result_path


def run_async_in_worker(coroutine_factory):
    """在全新线程/事件循环中执行异步 browser-use，隔离 CloakBrowser 的同步 Playwright loop。"""
    result = queue.Queue(maxsize=1)

    def target():
        try:
            result.put((True, asyncio.run(coroutine_factory())))
        except BaseException as error:  # 原样带回主线程，由 CLI 正常报错退出
            result.put((False, error))

    worker = threading.Thread(target=target, name="polaris-browser-use", daemon=False)
    worker.start()
    worker.join()
    ok, value = result.get()
    if not ok:
...
async def self_test_event_loop_worker():
    """模拟调用线程已有运行中 loop，证明 browser-use coroutine 会去新线程。"""
    return run_async_in_worker(lambda: asyncio.sleep(0, result="ok"))
...
async def run_agent(task, start_url, out_dir, max_steps, cdp_url, model_override):
    ...
    try:
        ...
        final, result_path = write_result(history, out_dir)
...
def main():
    ...
    ap.add_argument("task", nargs="?", help="高层任务描述(大白话)")
    ...
    ap.add_argument("--self-test-event-loop", action="store_true", help=argparse.SUPPRESS)
    a = ap.parse_args()

    if a.self_test_event_loop:
        print(f"event-loop-worker: {asyncio.run(self_test_event_loop_worker())}")
        return
    if not a.task:
    ...
    try:
        ...
        run_async_in_worker(
            lambda: run_agent(a.task, a.start_url, a.out, a.max_steps, cdp_url, a.model)
        )
    finally:
