...
// ─────────────── Claude 官方订阅 OAuth(对话框内复刻 `claude setup-token`)───────────────
// `claude setup-token` 是个需要 PTY 的全屏 TUI, 容器/Web 里跑不顺。这里用同一套官方 OAuth
// 参数(client_id/redirect_uri/scope 与 setup-token 实测一致)在 Polaris 对话框内重走 PKCE:
//   ① claude_login_start  生成 verifier/challenge(S256)+state, 暂存, 返回授权 URL(并试开浏览器)
//   ② 用户在浏览器授权 → 落地页显示形如 `CODE#STATE` 的码, 复制粘回对话框
//   ③ claude_login_submit 用 verifier 换 access/refresh token, 写 .credentials.json
// 关键: 换来的凭证带 refreshToken → claude 之后能自动续期, 根治「容器内 token 过期即 401」。
...
// authorize URL 里 redirect_uri / scope 含 `:` `/` 须百分号编码; 其余参数(client_id=uuid、
// challenge/state=base64url)字符集本就 URL 安全。token 交换走 send_form, 用未编码原值即可。
const CLAUDE_OAUTH_REDIRECT_ENC: &str =
    "https%3A%2F%2Fplatform.claude.com%2Foauth%2Fcode%2Fcallback";
const CLAUDE_OAUTH_SCOPE: &str = "user:inference";
const CLAUDE_OAUTH_SCOPE_ENC: &str = "user%3Ainference";

#[derive(Clone)]
struct ClaudeLoginPending {
    verifier: String,
    state: String,
}
static CLAUDE_LOGIN_PENDING: Lazy<RwLock<Option<ClaudeLoginPending>>> =
...
/// 生成 PKCE 的 (code_verifier, code_challenge=S256)。verifier = 32 字节 OS 随机熵 → 43 字符 base64url。
fn pkce_pair() -> (String, String) {
    let mut raw = [0u8; 32];
    // getrandom 失败极罕见(无 OS 熵源); 兜底混入纳秒时间, 授权短窗内仍不可预测。
    if getrandom::getrandom(&mut raw).is_err() {
        let t = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        raw[..16].copy_from_slice(&(t as u128).to_le_bytes());
    }
...
/// claude 凭证路径: 尊重 CLAUDE_CONFIG_DIR(容器内指向持久卷), 否则 ~/.claude/.credentials.json。
fn claude_credentials_path() -> Result<PathBuf, String> {
    ...
    UserDirs::new()
        .map(|u| u.home_dir().join(".claude").join(".credentials.json"))
...
/// ① 生成授权 URL(PKCE + state), 暂存 verifier/state, 尝试开浏览器。
#[cfg_attr(feature = "desktop", tauri::command)]
pub fn claude_login_start() -> Result<ClaudeLoginStart, String> {
    let (verifier, challenge) = pkce_pair();
    let state = rand_token(16);
    let url = format!(
        "{base}?code=true&client_id={cid}&response_type=code&redirect_uri={redir}\
        ...
        redir = CLAUDE_OAUTH_REDIRECT_ENC,
        scope = CLAUDE_OAUTH_SCOPE_ENC,
        chal = challenge,
        state = state,
    );
    *CLAUDE_LOGIN_PENDING.write() = Some(ClaudeLoginPending { verifier, state });
    // 桌面端便利地自动开浏览器; Docker headless 失败无害(UI 仍展示 URL 供手动打开)。
    let _ = codex_open_browser(&url);
    Ok(ClaudeLoginStart { url })
}
...
/// ② 用粘贴的 code(形如 `CODE#STATE`)换 token 并写 .credentials.json(带 refreshToken)。
#[cfg_attr(feature = "desktop", tauri::command)]
pub fn claude_login_submit(code: String) -> Result<ClaudeLoginResult, String> {
    let pending = CLAUDE_LOGIN_PENDING
        .read()
        .clone()
        .ok_or_else(|| "登录会话已失效, 请重新点「订阅登录」".to_string())?;
    let trimmed = code.trim();
    if trimmed.is_empty() {
        return Err("授权码为空".into());
    }
    // setup-token 落地页给的是 `CODE#STATE`; 按 `#` 切, `#` 后是回传 state。
    let (auth_code, state) = match trimmed.split_once('#') {
        Some((c, s)) => (c.trim().to_string(), s.trim().to_string()),
        None => (trimmed.to_string(), pending.state.clone()),
    };
    let resp = codex_agent()
        ...
        .map_err(|e| format!("Claude 授权换取 token 失败: {}", codex_http_err(e)))?;
    let tok: ClaudeTokenResp = resp
        .into_json()
        .map_err(|e| format!("解析 token 响应失败: {e}"))?;

    let refresh = tok.refresh_token.clone().unwrap_or_default();
    if refresh.is_empty() {
        // 没 refreshToken = 又会过期失联 → 明确报错, 别静默写半截凭证。
        return Err("授权成功但服务端未返回 refreshToken, 凭证无法自动续期; 请重试授权。".into());
    }
    let expires_in = tok.expires_in.unwrap_or(28800);
    ...
    {
        let _io = IO_LOCK.lock();
        atomic_write(&path, &serde_json::to_string_pretty(&blob).unwrap_or_default())
    ...
    *CLAUDE_LOGIN_PENDING.write() = None;
    Ok(ClaudeLoginResult {
        ok: true,
        email: tok.account.and_then(|a| a.email_address),
...
/// 读 .credentials.json 报告 OAuth 登录态(供 UI 显示「已登录 / 将过期 / 可自动续期」)。
#[cfg_attr(feature = "desktop", tauri::command)]
pub fn claude_auth_status() -> Result<ClaudeAuthStatus, String> {
    let path = claude_credentials_path()?;
    let mut st = ClaudeAuthStatus {
        logged_in: false,
        has_refresh: false,
        expired: false,
        expires_at: 0,
    ...
    let Some(o) = v.get("claudeAiOauth") else {
        return Ok(st);
    };
    let access = o.get("accessToken").and_then(|x| x.as_str()).unwrap_or("");
    st.logged_in = !access.is_empty();
    st.has_refresh = o
        .get("refreshToken")
        .and_then(|x| x.as_str())
        .map(|s| !s.is_empty())
        .unwrap_or(false);
    st.expires_at = o.get("expiresAt").and_then(|x| x.as_u64()).unwrap_or(0);
    let now_ms = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|d| d.as_millis() as u64)
        .unwrap_or(0);
    st.expired = st.expires_at != 0 && st.expires_at <= now_ms;
    Ok(st)
}
